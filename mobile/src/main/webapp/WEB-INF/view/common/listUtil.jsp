...
sy.createDatagrid = function(t,srchForm) {//t为table对象
	var isInit = true;
	var intiPageSize = parseInt((window.document.body.clientHeight-86)/25)+1;//大概的计算，不精确
	t.datagrid({
...
//渠道下拉框
function getLegalChannel(s,flag) {
	/*  id：节点ID，对加载远程数据很重要。
		text：显示节点文本。
	...
	s.combotree({
		url:postPath+"/channelBase/getLegalChannel?flag="+flag,
		method:'get',
		panelHeight:'auto',
		lines:true,
		//checkbox: true, //在tree上有效（能多选），在combotree上无效
		multiple:true,
		/* data: [{//静态读取数据
			text: 'Item1',
			state: 'open',
