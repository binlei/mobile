//查询数据按钮
function tj_search(chartContainer,subtitle,uri,tableContainer,type) {
	$(".dataSeries").show();
	$(".topN").hide();
	setTime(type);
	loadData(chartContainer,subtitle,uri,tableContainer);
}
//历史最高按钮
function historyMax(chartContainer,subtitle,uri,tableContainer) {
	$(".dataSeries").hide();
	$(".topN").show();
	loadData(chartContainer,subtitle,uri,tableContainer);
}
//右键事件
function rigntClick() {
	$("#dataTable,#topNTable").mousedown(function(e){ 
			 $('body').bind("contextmenu",function(e){
		         return false;
		    }); 
	      if(3 == e.which){ 
	    	   $('#menu').menu('show', {
	              left: e.pageX,
	              top: e.pageY
	          });  
		    return false;
	      }
	    }); 
}
//导出EXCEL
function exp2excel(url) { 
	var trs = $("#content").children("div :visible").eq(1).children("table").find("tr");
	var head = "";
	var field = "";
	for(var i=0;i<trs.length;i++) {
		var tds = trs.eq(i).find("td");
		for(var j=0;j<tds.length;j++) {
			if(i == 0) {
				head +=  tds.eq(j).html()+",";
			} else if(i == 1) {
				field += tds.eq(j).html()+",";
			}
		}
	}
	head = head.substring(0, head.length-1);
	field = field.substring(0, field.length-1);
	window.location.href = postPath+'/'+url+'/exp2excel?head='+head+"&field="+field;
}

//设置查询时间
function setTime(type) {
	if(type == 'day') {
		var startTime = ""+$("#uploadTime_from").datebox("getValue")+" 00:00:00";
		$("#startTime").val(startTime);
		var endTime = "";
		var uploadTime_to = new Date($("#uploadTime_to").datebox("getValue"));
		uploadTime_to.setDate(uploadTime_to.getDate()+1);
		endTime +=uploadTime_to.format("yyyy-MM-dd")+" 00:00:00";
		$("#endTime").val(endTime);
	} else if(type == 'week') {
		
	}else if(type == 'month') {
		var startTime = ""+$("#mYearStart").val()+"-"+$("#mMonthStart").val()+"-01 00:00:00";
		$("#startTime").val(startTime);
		var endTime = "";
		var mYearEnd = $("#mYearEnd").val();
		var mMonthEnd = parseInt($("#mMonthEnd").val())+1;
		if(mMonthEnd==13) {
			mYearEnd = parseInt(mYearEnd)+1;
			mMonthEnd = 1;
		}
		endTime +=mYearEnd+"-"+mMonthEnd+"-01 00:00:00";
		$("#endTime").val(endTime);
	}else if(type == 'season') {
		var startTime = ""+$("#sYearStart").val()+"-"+getStartMonthBySeason($("#sSeasonStart").val())+"-01 00:00:00";
		$("#startTime").val(startTime);
		var endTime = "";
		var sYearEnd = $("#sYearEnd").val();
		var sSeasonEnd = parseInt($("#sSeasonEnd").val())+1;
		if(sSeasonEnd == 5) {
			sYearEnd += 1;
			endTime += sYearEnd+"-01-01 00:00:00";
		} else {
			endTime += sYearEnd+"-"+getStartMonthBySeason(sSeasonEnd)+"-01 00:00:00";
		}
		$("#endTime").val(endTime);
	}else if(type == 'year') {
		var startTime = ""+$("#mYearStart").val()+"-01-01 00:00:00";
		$("#startTime").val(startTime);
		var endTime = "";
		var mYearEnd = parseInt($("#mYearEnd").val())+1;
		endTime +=mYearEnd+"-01-01 00:00:00";
		$("#endTime").val(endTime);
	}
	
}

//根据季度值返回该季度初始月份
function getStartMonthBySeason(n) {
	if(n == 1) {
		return "01";
	}else if(n == 2) {
		return "04";
	}else if(n == 3) {
		return "07";
	}else if(n == 4) {
		return "10";
	}
}